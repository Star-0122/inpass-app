...
-- チケット（1ユーザーが同じイベントのチケットを複数枚取得できる。
-- 一意性は ticket_code の unique 制約で保証する）
create table if not exists public.tickets (
  id uuid primary key default gen_random_uuid(),
  ticket_code text not null unique,           -- 表示・QR用の人間が読めるコード（INP-2026-XXXXXXXX）。1枚ごとに必ず異なる値
  event_id uuid not null references public.events(id) on delete cascade,
...
-- update は許可しない（チェックインは check_in_ticket() 関数経由のみ）
-- delete は「まだ使用されていない（valid）」自分のチケットのみ許可（監査性のため used は削除不可）
